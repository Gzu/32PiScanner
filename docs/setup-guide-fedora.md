# 32PiScanner — Full Setup Guide · **Fedora** field brain + Windows RC desktop

End-to-end bring-up for the rig with the **split architecture**:

- **Linux laptop (Fedora)** = portable field brain — serves **DHCP + NTP + SMB** to the rig.
...
> Running Ubuntu/Debian on the laptop instead? Use **`setup-guide-ubuntu.md`**.
...
| Host | IP | Role |
|---|---|---|
| Fedora laptop (rig NIC) | `192.168.50.1` static | DHCP + NTP + SMB |
...
## 1. Fedora laptop — the field brain

Assumes **Fedora Workstation** (39/40/41+) — NetworkManager, systemd-resolved,
firewalld, and **SELinux enforcing** by default. **The laptop needs internet for
`dnf` in this step**; it does not need internet in the field afterward.
...
sudo RIG_NIC=<your-wired-iface> SMB_PASS=<choose-a-password> ./setup-fieldbrain-fedora.sh
```
...
> Fedora differs from Debian/Ubuntu in four places that bite: package manager
> (`dnf`), chrony config path (`/etc/chrony.conf`) and service name (`chronyd`),
> firewall (`firewalld`), and **SELinux** — which will block Samba from serving
> `/srv/scans` unless you set the file context (§1.6). All handled below.

### 1.1 Packages

```bash
sudo dnf install -y chrony dnsmasq samba samba-client policycoreutils-python-utils
```

(`samba-client` gives you `smbclient` for the verify step; `policycoreutils-python-utils`
gives you `semanage` for the SELinux context in §1.6.)
...
Replace **`/etc/chrony.conf`** (Fedora path — note: *not* `/etc/chrony/chrony.conf`,
that's the Debian layout used on the Pis) with a server config (mirrors your repo's
`provision/chrony-server-pi.conf`, adapted for the laptop):
...
```bash
sudo systemctl enable --now chronyd    # Fedora service is 'chronyd', not 'chrony'
...
**SELinux (Fedora — do NOT skip this).** `/srv/scans` is outside the default Samba
paths, so SELinux blocks smbd from reading/writing it until you label it
`samba_share_t`. Symptom if skipped: uploads fail with `NT_STATUS_ACCESS_DENIED`
even though Unix permissions look correct.

```bash
sudo semanage fcontext -a -t samba_share_t "/srv/scans(/.*)?"
sudo restorecon -Rv /srv/scans
```
...
```bash
sudo systemctl enable --now smb        # Fedora service is 'smb' (not 'smbd')
...
Fedora runs **firewalld**. The rig LAN is a trusted, isolated subnet (this matches
the project's trust model: UDP broadcast on a closed LAN), so the simplest correct
move is to drop the **rig NIC into the `trusted` zone** — this allows everything on
that interface (DHCP, NTP, SMB, the UDP/9999 protocol, and the ephemeral-port replies
`cli.py` needs) without per-port fiddling:

```bash
sudo firewall-cmd --permanent --zone=trusted --change-interface=<RIG_NIC>
sudo nmcli con mod rig connection.zone trusted   # make it stick across reconnects
sudo firewall-cmd --reload
```

If you prefer least-privilege on the default zone instead of trusting the interface,
open the named services:

```bash
sudo firewall-cmd --permanent --add-service=ntp     # UDP/123
sudo firewall-cmd --permanent --add-service=dhcp    # UDP/67
sudo firewall-cmd --permanent --add-service=samba   # 137-139,445
sudo firewall-cmd --permanent --add-port=9999/udp   # picam protocol
sudo firewall-cmd --reload
```

> Note the ephemeral-reply gotcha: `cli.py` sends UDP broadcast and receives unicast
> replies on a random source port. firewalld's default zone won't allow those back in
> just because you opened 9999. If pings go out but no PONGs return, that's why —
> use the `trusted`-zone approach above.
...
cd ~/32PiScanner/provision
sudo NTP_SERVER=192.168.50.1 ./install.sh  # ← the Fedora laptop's rig IP
```
...
1. Cable: Fedora laptop + all 32 Pis into the **Gigabit switch**. No internet needed.
...
settings for field use. (On Fedora, also add masquerade to firewalld:
`sudo firewall-cmd --permanent --zone=trusted --add-masquerade && sudo firewall-cmd --reload`.)
...
|---|---|---|
| `clock_unsynced` from CAPTURE | chrony not converged, or Pi can't reach `192.168.50.1:123` | `chronyc tracking` on a Pi; check laptop's `chronyd` is running + firewalld allows NTP (rig NIC in `trusted` zone). Wait 30–60 s. |
...
| `upload_failed: NT_STATUS_ACCESS_DENIED` | **SELinux context missing** on `/srv/scans` (most common on Fedora), or dir not writable | `sudo semanage fcontext -a -t samba_share_t "/srv/scans(/.*)?" && sudo restorecon -Rv /srv/scans`; then `sudo chown scanner:scanner /srv/scans`. |
